...
async function main() {
  ...
  try {
    ...
    await assert.rejects(submitSkillAssessment('integration-first', { ...evidence, mode: 'challenge', challengeId: 5 }), error => error.statusCode === 409)
    ...
    console.log('Skill Hub integration passed: reviewer auth, blind queue, exclusive claim, rubric, verification, renewal, upgrade, archive/restore, daily practice, challenge, streak, gap report, TrustScore and transaction rollback.')
  } finally {
    try {
