...
test('review -> selection -> start -> delivery -> revision -> approval -> payment is shared by both accounts', async t => {
  ...
  assert.equal(company.projectWorkspaceState.projects.length, 1)
  assert.equal(company.projectWorkspaceState.projects[0].submissionId, id)
  assert.equal(company.projectWorkspaceState.projects[0].submissionStatus, 'selected')
  assert.equal(company.projectWorkspaceState.projects[0].status, 'Planning')
  ...
  assert.equal(company.projectWorkspaceState.projects[0].submissionStatus, 'work_started')
  assert.equal(company.projectWorkspaceState.projects[0].status, 'In Progress')
  ...
  assert.equal((await getStudentGigState('student-session')).activeGigBase.filter(item => !item.demoData).length, 1)
  ...
  assert.equal(earnings.pending.filter(item => !item.demoData).length, 1)
  ...
  assert.equal(earnings.pending.filter(item => !item.demoData).length, 0)
...
test('legacy wallet balances cannot be withdrawn or edited', async t => {
  ...
  const earningState = await getStudentEarningState('s')
  assert.deepEqual({ mode: earningState.mode, pending: earningState.pending, transactions: earningState.transactions, totalRecorded: earningState.totalRecorded }, { mode: 'external', pending: [], transactions: [], totalRecorded: 0 })
  assert.ok(earningState.demoTransactions.length > 0)
  assert.ok(earningState.demoTransactions.every(item => item.demoData))
