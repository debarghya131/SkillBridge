...
export default function PublicStudentProfile({ profile, onClose, action, loading = false }) {
  ...
  // Until the student publishes a recording, show the account starter video.
  ...
  return <div className="public-profile-overlay" onMouseDown={e => e.target === e.currentTarget && onClose()}>
    <section className="public-profile" role="dialog" aria-modal="true" aria-label={`${profile.name} profile`}>
      <header>{profile.avatar ? <button type="button" className="public-profile-avatar public-profile-avatar-button" aria-label={`View ${profile.name}'s profile photo`} title="View profile photo" onClick={() => setPhotoOpen(true)}><img src={profile.avatar} alt={`${profile.name}'s profile`}/></button> : <div className="public-profile-avatar">{profile.name?.[0]}</div>}<div><h2>{profile.name}</h2><div className="public-profile-score"><span><Star size={15} fill="currentColor"/><strong>{profile.trustScore ?? 0}</strong><small>/ 1000</small></span><small>TrustScore</small></div><p className="public-profile-location"><MapPin size={13}/>{profile.location || 'Location not provided'}</p>
      ...
      <div className="public-profile-body">
        ...
        <section><h3><Video size={16}/>Intro video</h3><video controls preload="metadata" src={video}>Your browser cannot play this video.</video></section>
