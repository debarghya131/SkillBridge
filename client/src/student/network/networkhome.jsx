...
export default function NetworkHome() {
  ...
  return <div className="network-page">
    ...
    <div className="network-toolbar"><label><Search size={16}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search name, location, or skill" /></label><select value={skill} onChange={event => setSkill(event.target.value)} aria-label="Filter by skill">{skills.map(item => <option key={item}>{item}</option>)}</select></div>
