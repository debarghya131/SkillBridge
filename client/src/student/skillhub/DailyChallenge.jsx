  ...
  return <section className="sh-habit-tracker" aria-labelledby="practice-streak-title">
    <header className="sh-habit-header">
      ...
      <div className="sh-habit-stats" aria-label="Practice totals">
        ...
        <span className="sh-metric-tooltip" tabIndex={0} data-tooltip="Verified skills with an active practice streak." aria-label="Active skills"><Target size={15}/><strong>{streaks.activeSkills || 0}</strong> active skills</span>
      </div>
    </header>
      ...
      <div className="sh-habit-grid" style={{ '--habit-days': daysInMonth }}>
        ...
        {ranked.map(skill => {
          ...
          return <div className="sh-habit-row" key={skill.name}>
            <div className="sh-habit-skill"><div><strong>{skill.name}</strong><span>{skill.category} · {skill.stage}</span></div><div className="sh-habit-skill-meta"><span><Flame size={13}/>{current} days</span>{eligible ? <small>Active</small> : <small className="is-locked">{skill.archived ? 'Archived' : 'Verify first'}</small>}</div></div>
  ...
  return <div className="sh-daily-layout">
    ...
    <section className="sh-panel"><header className="sh-section-heading"><div><span className="sh-section-kicker">Evidence tasks</span><h2>Challenges</h2></div><span>+{daily.points || 0} Trust for today's submissions</span></header>
      ...
      <div className="sh-list">{challenges.map(challenge => {
        const skill = skills.find(item => !item.archived && item.name.toLowerCase() === challenge.skill.toLowerCase())
