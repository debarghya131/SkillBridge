...
import { ExternalLink, Flame, Send, ShieldCheck, X } from 'lucide-react'
    ...
    async function load() {
      try {
        const [result, overview, submissions, payments] = await Promise.all([
          ...
          fetchCompanyTaskSubmissions(token),
          active === 'payment' ? fetchCompanyPayment(token) : null,
        ])
        if (cancelled) return
        setBusinessProfile(mergeCompanyProfile(result.company.businessProfile || result.company))
        setContactMethod(result.company.contactMethod || 'email')
        setVerificationMethod(result.company.verificationMethod || 'gstin')
        ...
        setTaskLibraryState(mergeCompanyTaskLibraryState(result.company.taskLibraryState))
        setTaskSubmissions((submissions.taskSubmissions || []).filter(item => item?.id))
        if (overview) setDashboardOverview(overview.dashboard)
        if (payments) setPaymentState(mergeCompanyPaymentState(payments.paymentState))
      } catch (error) {
        if (cancelled) return
        if (error.status === 401) {
          clearCompanySessionToken()
            ...
            <div>
                ...
                <div className="responsive-filter-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 1.1fr) 1.2fr 1fr 1fr auto', gap: 10, alignItems: 'end', marginBottom: 10 }}>
                  ...
                  <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    <select
                      ...
                      onChange={e => updateTalentFilters({ location: e.target.value })}
                  ...
                  <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    <select
                      ...
                      onChange={e => updateTalentFilters({ skill: e.target.value })}
                      style={{ padding: '8px 10px', borderRadius: 8, border: '1px solid var(--border)', fontSize: 12, background: 'var(--white)', fontFamily: 'inherit' }}
                    >
              ...
              <div aria-busy={isTalentLoading} style={{ display: 'flex', flexDirection: 'column', gap: 12, opacity: isTalentLoading ? 0.65 : 1, transition: 'opacity 0.15s' }}>
                ...
                {talentProfiles.map((p, i) => (
                  <div className="responsive-hero" key={p.id} style={{
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (request === talentProfileRequest.current) setSelectedTalent({ ...result.profile, opportunityProfile: p })
